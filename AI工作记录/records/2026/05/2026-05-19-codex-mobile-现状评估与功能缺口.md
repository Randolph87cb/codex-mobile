...
## 追加记录：补充 AGENTS 目录结构维护规则

- 时间：2026-05-19
- 用户要求：
  - 在 `AGENTS.md` 中写明当前文件目录结构。
  - 明确要求后续如果目录结构发生变化，要同步更新这段说明。
...
- 文件：
  - `AGENTS.md`
- 修改内容：
  - 新增“当前目录结构”章节，列出当前仓库顶层目录树。
  - 增加维护规则：顶层目录新增、删除、重命名，或目录职责发生明显变化时，必须同步更新该节。
  - 增加补充规则：如果某个目录从本地资产变成正式协作目录，也要同步更新“当前代码面”和目录结构说明。

### 本轮验证

- 人工检查：
  - 读取仓库顶层目录，确认当前结构与 `AGENTS.md` 中的目录树一致。
  - 本轮未涉及 `bridge/` 或 `android/` 代码逻辑修改，因此未额外运行构建或测试。
