...
- 已同步更新 `docs/session-detail-ui-notes.md`：
  - 明确“回到前台时先补一次详情快照”的规则
  - 明确 Android 端现在通过显式待补标记，而不是 UI 状态启发式，来决定 `session.started` 后的补拉
- 已同步更新 `docs/architecture.md`：
  - 在 Android 职责中补充“前后台切换和实时流恢复后的详情快照追平”
- 本轮文档同步未新增构建或测试；沿用上一轮代码修改时已通过的 Android 单测与构建结果
