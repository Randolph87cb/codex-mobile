...
- `mobile_uploads/`: 手机上传图片和视频的本地保存目录，默认被 Git 忽略
- `AI工作记录/`: 项目内工作记录
...
- 已支持图片预上传、图片展示、预览和保存；
- 已支持视频预上传、会话内文件链接展示与下载；
- 手机端图片和视频在上传到 bridge 后，可按会话 `cwd` 自动保存到电脑侧 `mobile_uploads/`；
...
- Android 会话详情页已支持在线程标题右侧直接修改线程名称，修改后会同步刷新详情页和线程列表；
...
- Android 线程列表现在按最后一次对话活动时间倒序排列，并把时间显示压缩成聊天式的 `HH:mm / N天前 / N周前`；
...
1. Android 先把真实附件上传到 `bridge`。
2. `bridge` 先把附件写入临时附件目录。
3. 如果上传时已经带了真实 `sessionId`，`bridge` 会继续把附件正式保存到当前会话 `cwd/mobile_uploads/`。
4. 后续会话输入优先提交正式保存后的本地路径，而不是再次上传原始二进制。
...
- 同名文件会自动去重，不覆盖已有文件。
- 如果 Android 还处于草稿会话，首条带附件消息发送前会先创建真实会话，再补一次带 `sessionId` 的附件上传，以拿到正式保存路径。
...
- 图片附件预览与保存；
- 视频附件上传、正式保存、文件链接展示与下载；
- 手机端上传图片或视频后，bridge 可把原始文件正式保存到当前会话工作目录下的 `mobile_uploads/`；
...
- “终止当前轮”当前通过 `POST /api/session/:id/interrupt` 调用，客户端统一显式发送空 JSON body `{}`，避免空 `POST` 在 bridge 侧触发 `415 Unsupported Media Type`；
