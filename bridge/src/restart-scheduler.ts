...
function buildInnerHelperCommand(input: RestartHelperCommandInput): string {
  return [
    ...
    `try {`,
    `    & ${quotePowerShell(input.scriptPath)} -HostAddress ${quotePowerShell(input.host)} -Port ${input.port} -Runner ${quotePowerShell(input.runner)} *>> ${quotePowerShell(input.helperLogPath)}`,
