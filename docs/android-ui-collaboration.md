...
- `SessionDetailScreen` 已完成两轮细化：
  ...
  - transcript 消息颜色已抽到 `ui/theme/TranscriptColors.kt`，浅色 / 深色都要走统一 token
...
- 短消息按内容宽度显示，不强制铺满中间槽。
- 长消息受最大宽度约束，不能把头像槽挤掉。
- transcript 相关颜色不要继续写死在 `SessionDetailScreen.kt`。
- 消息气泡、执行过程卡、代码块、Markdown 链接和行内代码，统一从 `ui/theme/TranscriptColors.kt` 取色。
- 如果只调浅色或只调深色，另一套也要同步检查，避免再次出现“暗色正文前景和浅色气泡叠用”的低对比问题。
